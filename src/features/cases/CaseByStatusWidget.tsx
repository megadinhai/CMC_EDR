...
import { CASE_STATUS_COLORS } from "@/lib/colors";

export function CaseByStatusWidget() {
  const timeRange = useTimeRange();
  const { data, isLoading, isError } = useCasesWidgets(timeRange);
  const items = data?.caseByStatus ?? [];
  const total = items.reduce((sum, i) => sum + i.count, 0);
  const status = resolveWidgetStatus(isLoading, isError, total === 0);

  return (
    <WidgetContainer title="Case by Status" status={status} bodyClassName="flex h-full items-center gap-4">
      <div className="h-[216px] w-[216px] shrink-0">
        <DonutChart
          data={items.map((i) => ({
            name: i.status,
            value: i.count,
            color: CASE_STATUS_COLORS[i.status],
          }))}
        />
      </div>
