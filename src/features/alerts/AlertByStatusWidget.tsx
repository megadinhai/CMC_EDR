...
import { ALERT_STATUS_COLORS } from "@/lib/colors";

export function AlertByStatusWidget() {
  const timeRange = useTimeRange();
  const { data, isLoading, isError } = useAlertsWidgets(timeRange);
  const items = data?.alertByStatus ?? [];
  const total = items.reduce((sum, i) => sum + i.count, 0);
  const status = resolveWidgetStatus(isLoading, isError, total === 0);

  return (
    <WidgetContainer title="Alert by Status" status={status} bodyClassName="flex h-full items-center gap-4">
      <div className="h-[216px] w-[216px] shrink-0">
        <DonutChart
          data={items.map((i) => ({
            name: i.status,
            value: i.count,
            color: ALERT_STATUS_COLORS[i.status],
          }))}
        />
      </div>
